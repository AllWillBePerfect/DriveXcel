...
import org.my.drivexcel.ui.screens.event.utils.EventTabs
import org.my.drivexcel.ui.screens.event.utils.toName
import org.my.drivexcel.ui.screens.importt.ImportRoute
import org.my.drivexcel.ui.screens.users.UsersRoute

@Composable
fun EventRoute(
    eventId: String,
    viewModel: EventViewModel = koinViewModel(),
    onBackClickedNavigate: () -> Unit
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                EventUiEvent.NavigateBack -> onBackClickedNavigate()
            }
        }
    }
...
private fun EventContent(
    ...
    if (!uiState.isLoadingError) {
        Column(
            ...
            if (uiState.idState is EventIdUiState.Initialized) {
                when (EventTabs.entries[selectedTab]) {
                    EventTabs.Users -> {
                        UsersRoute(
                            eventId = uiState.idState.id
                        )
                    }
